            ...
            if (value !== undefined && value !== null && value !== "") {
                ...
                try {
                    ...
                    const formValue = field === "loanAmount" && typeof value === "number"
                        ? value.toString()
                        : value;
                    form.setFieldValue(field, formValue);
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex space-x-2">
            {["loan_amount", "loan_type", "personal_details"].map((step, index) => {
            ...
            return (<div key={step} className={`flex-1 h-2 rounded-full ${isCompleted
                    ? "bg-green-500"
                    : isCurrent
                        ? "bg-blue-500"
                        : "bg-gray-200"}`}/>);
        ...
        <Card className="h-[600px] flex flex-col">
          ...
          <CardContent className="flex-1 flex flex-col p-0 overflow-y-auto">
            <div className="flex-1 overflow-y-auto">
              <MessageList>
                {conversationState.messages.map((message) => (<Message key={message.id} role={message.role} content={message.content} timestamp={message.timestamp}/>))}
                {conversationState.isTyping && (<Message role="assistant" content="" isTyping={true}/>)}
                <div ref={messagesEndRef}/>
              </MessageList>
            </div>
            {shouldShowSubmitButton(((_a = conversationState.messages[conversationState.messages.length - 1]) === null || _a === void 0 ? void 0 : _a.content) || "", conversationState.formData, conversationState.currentStep) ? (<div className="p-4 border-t">
                <Button onClick={handleChatSubmission} disabled={isSubmitting} className="w-full h-12 text-lg font-semibold">
                  {isSubmitting ? (<div className="flex items-center gap-2">
                      <div className="h-4 w-4 animate-spin rounded-full border-2 border-current border-t-transparent"></div>
                      Submitting Application...
                    </div>) : ("Submit Application")}
                </Button>
              </div>) : (<ChatInput onSendMessage={handleSendMessage} disabled={conversationState.isTyping || isFormComplete} placeholder={conversationState.currentStep === "loan_amount"
                ? "Enter loan amount..."
                : conversationState.currentStep === "loan_type"
                    ? "Choose loan type..."
        ...
        <Card className="h-[600px] flex flex-col">
          ...
          <CardContent className="flex-1 flex flex-col">
            <div className="flex-1 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input value={conversationState.formData.loanAmount
            ? `$${conversationState.formData.loanAmount.toLocaleString()}`
            : ""} placeholder="Will be filled from conversation" disabled className="bg-gray-50"/>
                  {conversationState.formData.loanAmount && (<CheckCircle className="h-5 w-5 text-green-500"/>)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input value={conversationState.formData.loanType || ""} placeholder="Will be filled from conversation" disabled className="bg-gray-50"/>
                  {conversationState.formData.loanType && (<CheckCircle className="h-5 w-5 text-green-500"/>)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input value={conversationState.formData.name || ""} placeholder="Will be filled from conversation" disabled className="bg-gray-50"/>
                  {conversationState.formData.name && (<CheckCircle className="h-5 w-5 text-green-500"/>)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input value={conversationState.formData.email || ""} placeholder="Will be filled from conversation" disabled className="bg-gray-50"/>
                  {conversationState.formData.email && (<CheckCircle className="h-5 w-5 text-green-500"/>)}
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input value={conversationState.formData.phone || ""} placeholder="Will be filled from conversation" disabled className="bg-gray-50"/>
                  {conversationState.formData.phone && (<CheckCircle className="h-5 w-5 text-green-500"/>)}
                </div>
              </div>
            </div>
            ...
            <div className="pt-4 border-t">
              <Button onClick={() => form.handleSubmit()} className="w-full" disabled={!isFormComplete || isSubmitting}>
...
//# sourceMappingURL=ConversationalForm.jsx.map